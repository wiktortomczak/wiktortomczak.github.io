...
gflags.DEFINE_string('out_buffering', True, '')
    ...
    for step_id in itertools.count():
      ...
      if model_file:
        model_step = dict(
          ...
          cores=cores, periphery=periphery)
        json.dump(model_step, model_file)
        model_file.write('\n')
...
def OutFile():
  ...
  if FLAGS.out == '-':
    if FLAGS.out_buffering:
      ...
      return os.fdopen(sys.stdout.fileno(), 'w', 0)
  else:
    if FLAGS.out_buffering:
      return file(FLAGS.out, 'w')
    else:
      return file(FLAGS.out, 'w', 1)
